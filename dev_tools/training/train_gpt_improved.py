...
class Hyperparameters:
    ...
    # Quantization
    mlp_quant_bits = int(os.environ.get("MLP_QUANT_BITS", 5))  # int5 for MLP
...
def mixed_quantize(state_dict: dict[str, Tensor], mlp_bits: int, attn_bits: int):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in FP16_KEEP_NAME_PATTERNS) or t.numel() <= 65536:
            ...
            continue
        if cat == "mlp":
            clip = (1 << (mlp_bits - 1)) - 1  # 15 for int5, 7 for int4
            q, s = quantize_intN_per_row(t, clip_range=clip)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{mlp_bits}"}
        elif cat == "attn":
...
def main() -> None:
    ...
    log0(f"mlp_quant_bits:{args.mlp_quant_bits} attn_quant_bits:{args.attn_quant_bits}")
    ...
    quant_result, quant_meta = mixed_quantize(sd_cpu, args.mlp_quant_bits, args.attn_quant_bits)
    ...
    if master_process:
        ...
        log0(f"Serialized model int{args.mlp_quant_bits}/int{args.attn_quant_bits}+{_COMPRESSOR}: {quant_file_bytes} bytes")
